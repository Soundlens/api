...
class JSONSchema(Schema):
    ...
    def get_properties(self, obj) -> typing.Dict[str, typing.Dict[str, typing.Any]]:
        ...
        # check if it has a function __jsonschema_type_mapping if so we should return that
        if hasattr(obj, "_jsonschema_type_mapping"):
            return obj._jsonschema_type_mapping()
    ...
    def _dump_oneof_schema(self, obj, many=None, *args):
        """
        Handles serialization of OneOfSchema with _type present globally,
        but avoids adding it as an additional property to each individual schema.
        """
        schemas = []

        for type_name, schema in obj.type_schemas.items():
            # Dump each schema individually
            if isclass(schema):
                schema = schema()
            schema_dump = self.dump(schema, *args, many=many)

            print(schema.__name__ if isclass(schema) else schema.__class__.__name__, flush=True)
            try:
                schema_dump = schema_dump["definitions"][
                    schema.__name__ if isclass(schema) else schema.__class__.__name__
                ]

            except KeyError:
                print(
                    f"KeyError: {schema.__name__ if isclass(schema) else schema.__class__.__name__}", flush=True
                )

            try:
                properties = schema_dump["properties"]
            except KeyError:
                properties = {}

            
            try:
                required = schema_dump["required"]
            except KeyError:
                required = []

            # Add the individual schema (without _type) to the list of schemas
            schemas.append(
                {"properties": properties, "required": required, "title": type_name}
            )

        # If only one schema, return it directly with _type included at the top level
        if len(schemas) == 1:
            # Inject _type into the schema at the top level
            schema = schemas[0]
            schema["properties"]["_type"] = {
                "type": "string",
                "enum": [schemas[0]["title"]],  # Set the enum to the schema's type name
                "default": schemas[0]["title"],  # Set the default value
                "readOnly": True,  # Or 'hidden': True if supported by the form generator
            }
            # schema['required'].append('_type')  # Ensure _type is required
            return schema

        # If multiple schemas, return the oneOf structure with _type at the top level
        sc = {
            "type": "object",
            "properties": {
                "_type": {
                    "type": "string",
                    "enum": [
                        schema["title"] for schema in schemas
                    ],  # Enum of all types
                    "readOnly": True,  # Make it read-only or hidden
                }
            },
            "oneOf": schemas,
            "required": ["_type"],
    ...
    def _get_python_type(self, field):
        ...
        MARSHMALLOW_TO_PY_TYPES_PAIRS.append((GenericLoadDumpField, dict))
    ...
    def _get_schema_for_field(self, obj, field):
        ...
        if hasattr(field, "_jsonschema_type_mapping"):
            ...
            if isinstance(field, fields.Nested):
                ...
                # TODO: check if it is a load or dump schema
                schema = field.load_schema_class()
                if hasattr(schema, "type_schemas"):
                    schema = self._dump_oneof_schema(schema, many=field.many)
    ...
    def _from_nested_schema(self, obj, field):
        ...
        # check if is GenericLoadDumpField
        if isinstance(field, GenericLoadDumpField):

            # how to know if it is a load or dump schema?

            nested = field.load_schema_class()
            if hasattr(nested, "type_schemas"):
                return self._dump_oneof_schema(nested, many=field.many)
        ...
        # check if it has a function __jsonschema_type_mapping if so we should return that
        if hasattr(nested, "_jsonschema_type_mapping"):
            return nested._jsonschema_type_mapping()
    ...
    def dump(self, obj, **kwargs):
        ...
        # print("SCHEMA", obj, flush=True)
        # print("FIELDS", obj.fields, flush=True)
